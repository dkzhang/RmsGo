...
func (wf Workflow) SystemApply(form generalForm.GeneralForm, userInfo user.UserInfo) (appID int, waErr webapiError.Err) {
	...
	theApplication := application.Application{
		...
		ApplicantUserID:          user.SystemUserAuto.UserID,
		ApplicantUserChineseName: user.SystemUserAuto.ChineseName,
		DepartmentCode:           user.SystemUserAuto.DepartmentCode,
	...
	theAppOpsRecord := application.AppOpsRecord{
		...
		ProjectID:          projectID,
		ApplicationID:      appID,
		OpsUserID:          user.SystemUserAuto.UserID,
		OpsUserChineseName: user.SystemUserAuto.ChineseName,
		Action:             application.AppActionSubmit,
		ActionStr:          "系统自动生成并发起",
